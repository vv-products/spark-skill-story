...
export function StudioLayout({ children, title, actions }: { children: ReactNode; title: string; actions?: ReactNode }) {
  ...
  return (
    <div className="studio-root flex min-h-screen w-full">
      {/* Sidebar */}
      <aside className="flex w-[240px] flex-col bg-[#1A1A2E] text-[#AAAACC]">
      ...
      <div className="flex min-w-0 flex-1 flex-col bg-[#F8F8FC]">
        <header className="sticky top-0 z-10 flex h-[60px] items-center justify-between border-b border-[#EBEBF5] bg-white px-6">
          <h1 className="text-[18px] font-bold text-[#1A1A2E]">{title}</h1>
          <div className="flex items-center gap-3">
            {unsaved && (
